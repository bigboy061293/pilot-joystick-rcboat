...
float   fBearMag;       // Bearing joystick: it is in degree, must be mapped from [MIN_SBUS,MAX_SBUS] -> [0,90]
...
void loop() {
  ...
  if (sbus_rx.Read()) {
    ...
    if (sbus_rx.rx_channels()[CHANNEL_MODE] >= CHANNEL_MODE_RC) {
      ...
      //fBearMag = map(sbus_rx.rx_channels()[3],MIN_SBUS,MAX_SBUS,0,90); // Bearing, Z axis from 0 degree to 90
      ...
      if (nJoyY == 0) {
        ...
        //fBearMag = map(sbus_rx.rx_channels()[3],MIN_SBUS,MAX_SBUS,-90,90); // Bearing, Z axis from 0 degree to 90
        //fBearMag = abs(fBearMag);
      }
      Serial.print("fBearMag: ");
        Serial.println(fBearMag);
      ...
      if (DEBUG_SERIAL) {
        Serial.println("____Differential Mode (Bearing)____");
